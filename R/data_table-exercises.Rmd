...
#or try (though not sure why I'd try since this seems longer)
thestates <- c("New York", "Texas")
murders[state %in% thestates]
```
...
Make sure `murders` has been defined with `rate` and `rank` and still has all states. Create a table called `my_states` that contains rows for states satisfying both the conditions: they are in the Northeast or West and the murder rate is less than 
```{r}
nrow(murders)
names(murders)

my_states <- murders[region %in% c("West", "Northeast") & rate < 1]
```

1.  Show only the state name, the rate, and the rank.
```{r}
...
ref <- dta[AgeDecade == " 20-29", .(avg = mean(BPSysAve, na.rm=TRUE), standard_deviation = sd(BPSysAve, na.rm=TRUE))]
ref
...
```{r}
dta[AgeDecade == " 20-29", .(min=min(BPSysAve, na.rm=TRUE), max=max(BPSysAve, na.rm=TRUE))]
...
```{r}
dta[Gender=="female", .(avg=mean(BPSysAve, na.rm=TRUE), std_dev=sd(BPSysAve, na.rm=TRUE)), by=AgeDecade]
```


4\. Repeat exercise 3 for males. 
```{r}
dta[Gender=="male", .(avg=mean(BPSysAve, na.rm=TRUE), std_dev=sd(BPSysAve, na.rm=TRUE)), by=AgeDecade]
...
```{r}
d<- dta[Gender=="male", .(avg=mean(BPSysAve, na.rm=TRUE), std_dev=sd(BPSysAve, na.rm=TRUE)), by=Race1]
d[order(avg)] |> head()
