...
file.copy(fullpath, "murders-notes.csv")
```


```{r, eval=FALSE}
library(tidyverse)
dat <- read_csv("murders-notes.csv")
```
...
wd <- getwd()
```
if you want to set the working directory, can setwd("WHEREVER YOU ARE")

if you don't set wd, can use the full file name: read_csv("full file path")

get full path of current dataset/file: fullpath
...
```{r}
read_lines("murders-notes.csv", n_max = 3)
```
we see that it's separated by commas, use CSV
...
```{r, message = FALSE}
dat <- read_csv(url) #can read by putting url and file name
```


```{r}
download.file(url, "murders-notes-2.csv") #doswnloads file and calls it this in the home directory
```
...
common problem - you think you have a text file (ASCII) but it's not, it's UNICODE (common when you have accents, etc.)
