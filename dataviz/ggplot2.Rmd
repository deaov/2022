...
```{r, eval = FALSE}
murders |> ggplot() + 
  geom_point(aes(x = population/10^6, y = total)) #anything mapped on plot from data needs 
                                                  #to go inside aes()
```
 

Using already defined `p`; p is already object associated with murders dataset, can add layers :)!
...
size=3 makes points bigger
```{r ggplot-example-5}
p + geom_point(aes(population/10^6, total), size = 3) +
  geom_text(aes(population/10^6, total, label = abb))
```

don't want abbreviation to be on top of the points:
```{r ggplot-example-6}
p + geom_point(aes(population/10^6, total), size = 3) +
  geom_text(aes(population/10^6, total, label = abb), nudge_x = 1.5)#NOTE THIS IS OUTSIDE AES(); not getting it from the data, this is a decision you're making
```
...
```{r}
p <- murders |> 
  ggplot(aes(population/10^6, total, label = abb)) #add mapping right at the beginning
p
```
...
COLOR IS A MAPPING BECAUSE IT'S COMING FROM THE REGION
